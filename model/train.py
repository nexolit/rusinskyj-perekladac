...
# Data parameters
data_folder = 'corpus/'  # folder with data files
...
# Learning parameters
checkpoint = "models/transformer_checkpoint.pth.tar"  # path to previous model checkpoint, None if none
tokens_in_batch = 1000  # batch size in target language tokens
batches_per_step = 5000 // tokens_in_batch  # perform a training step, i.e. update parameters, once every so many batches
print_frequency = 10  # print status once every so many steps
n_steps = 100000  # number of training steps
warmup_steps = 10000  # number of warmup steps where learning rate is increased linearly; twice the value in the paper, as in the official transformer repo.
